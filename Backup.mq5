...
void MainCloseAll()
{
    if (closeAllTrigger == ENUM_CLOSE_ALL_TRIGGER::DISABLE)
        return;

    switch (closeAllTrigger)
    {
    case CLOSE_SYMBOL_BUY_SELL:
        for (int i = 0; i < totalSymbols; i++)
        {
            string symbol = symbolArray[i].symbol;
            if (triggerType == ENUM_TRIGGER_TYPE::TRIGGER_POINTS)
                SymbolCloseAllByPoint(symbol, true, true, triggerValue);
            else if (triggerType == ENUM_TRIGGER_TYPE::TRIGGER_DOLLARS)
                SymbolOrAllCloseByDollar(symbol, true, true, triggerValue);
        }
        break;

    case CLOSE_ALL_SYMBOL_BUY:
        for (int i = 0; i < totalSymbols; i++)
        {
            string symbol = symbolArray[i].symbol;
            if (triggerType == ENUM_TRIGGER_TYPE::TRIGGER_POINTS)
                SymbolCloseAllByPoint(symbol, true, false, triggerValue);
            else if (triggerType == ENUM_TRIGGER_TYPE::TRIGGER_DOLLARS)
                SymbolOrAllCloseByDollar(symbol, true, false, triggerValue);
        }
        break;
    case CLOSE_ALL_SYMBOL_SELL:
        for (int i = 0; i < totalSymbols; i++)
        {
            string symbol = symbolArray[i].symbol;
            if (triggerType == ENUM_TRIGGER_TYPE::TRIGGER_POINTS)
                SymbolCloseAllByPoint(symbol, false, true, triggerValue);
            else if (triggerType == ENUM_TRIGGER_TYPE::TRIGGER_DOLLARS)
                SymbolOrAllCloseByDollar(symbol, false, true, triggerValue);
        }
        break;
    case CLOSE_ALL_SYMBOL:

        if (triggerType == ENUM_TRIGGER_TYPE::TRIGGER_DOLLARS)
            for (int i = 0; i < totalSymbols; i++)
            {
                string symbol = symbolArray[i].symbol;
                SymbolOrAllCloseByDollar(symbol, true, true, triggerValue);
            ...
            Print("Trigger type Harus Dollar");
        break;
    case CLOSE_ALL:
        if (triggerType == ENUM_TRIGGER_TYPE::TRIGGER_DOLLARS)
            SymbolOrAllCloseByDollar(NULL, true, true, triggerValue);
        else
            Print("Trigger type Harus Dollar");
        break;
    case CLOSE_ALL_BUY:
        if (triggerType == ENUM_TRIGGER_TYPE::TRIGGER_DOLLARS)
            SymbolOrAllCloseByDollar(NULL, true, false, triggerValue);
        else
            Print("Trigger type Harus Dollar");
        break;
    case CLOSE_ALL_SELL:
        if (triggerType == ENUM_TRIGGER_TYPE::TRIGGER_DOLLARS)
            SymbolOrAllCloseByDollar(NULL, false, true, triggerValue);
        else
            Print("Trigger type Harus Dollar");
        break;
    }
