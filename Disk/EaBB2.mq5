...
void Main()
{
    ...
    for (int i = 0; i < total; i++)
    {
        ...
        ArrayResize(upper_band, copydata);
        ArrayResize(lower_band, copydata);
        ArrayResize(close_array, 1);

        int copy = CopyClose(symbol, PERIOD_M1, 0, 1, close_array);
        if (CopyBuffer(g_HandleBB[i], 1, 0, copydata, upper_band) != copydata ||
            CopyBuffer(g_HandleBB[i], 2, 0, copydata, lower_band) != copydata)
        {
        ...
        upperband = NormalizeDouble(upper_band[0], (int)digit);
        lowerband = NormalizeDouble(lower_band[0], (int)digit);
        close = close_array[0];

        // Print("✔️[EaBB2.mq5:217]: ", position.Select(symbol));
        //  Print("✔️[EaBB2.mq5:218]: ", symbol, " ", position.SelectByMagic(symbol, 123456));

        //  CheckMarketData(symbol);
        // Print("✔️[EaBB2.mq5:221]: ", terminalInfo.IsTradeAllowed());
        // Print("✔️[EaBB2.mq5:222]: ", terminalInfo.CPUCores());
        // Print("✔️[EaBB2.mq5:223]: ", terminalInfo.DiskSpace());
        // Print("✔️[EaBB2.mq5:224]: ", terminalInfo.IsConnected());
        // Print("✔️[EaBB2.mq5:225]: ", terminalInfo.Path());
        // Print("✔️[EaBB2.mq5:226]: ", terminalInfo.Company());

        string from_date = "2024.01.01 00:00:00";
        datetime to_date = TimeCurrent();
        // Print(from_date, " ", to_date);
        // return;

        HistorySelect((datetime)from_date, to_date);
        ...
        for (int i = 0; i < total; i++)
        {
            if (historyOrderInfo.SelectByIndex(i))
            {
                ...
                // Mendapatkan informasi order
                string symbol = historyOrderInfo.Symbol();
                double volume = historyOrderInfo.VolumeCurrent();
                double price = historyOrderInfo.PriceOpen();
                ENUM_ORDER_TYPE type = historyOrderInfo.OrderType();
                datetime openTime = historyOrderInfo.TimeSetup();
                string comment = historyOrderInfo.Comment();
                long magic = historyOrderInfo.Magic();

                // Print("History Order #", ticket,
                //       "\nSymbol: ", symbol,
                //       "\nVolume: ", volume,
                //       "\nPrice: ", price,
                //       "\nType: ", EnumToString(type),
                //       "\nOpen Time: ", TimeToString(openTime),
                //       "\nComment: ", comment,
                //       "\nMagic: ", magic);
            }
        }
...
void CheckMarketData(string symbol)
{
